...
class Dataset(object):
    ...
    def batch_sentences(self, sentences, lang_id):
        ...
        # if self.use_len:
        #     lengths = torch.LongTensor([len(s) + 3 for s in sentences])
        #     sent = torch.LongTensor(lengths.max(), lengths.size(0)).fill_(self.pad_index)

        #     sent[0] = self.bos_index[lang_id]
        #     for i, s in enumerate(sentences):
        #         sent[1:lengths[i] - 2, i].copy_(s)
        #         sent[lengths[i] - 2, i] = self.eos_index
        #     sent[-1].copy_(torch.LongTensor([len(s) + 2 for s in sentences]))
        #     # lengths = lengths - 1
        # else:
...
class MonolingualDataset(Dataset):
    ...
    def __init__(self, sent, pos, dico, lang_id, params, has_abs=False, sent_abs=None, pos_abs=None):
        super(MonolingualDataset, self).__init__(params)
        assert type(lang_id) is int
        self.sent = sent
        self.pos = pos
        if has_abs:
            self.has_abstact = True
            self.sent_abs = sent_abs
            self.pos_abs = pos_abs
        else:
            self.has_abstact = False
        self.dico = dico
    ...
    def remove_empty_sentences(self):
        ...
        if self.has_abstact:
            self.pos_abs = self.pos_abs[indices]
        self.lengths = self.pos[:, 1] - self.pos[:, 0]
    ...
    def remove_long_sentences(self, max_len):
        ...
        """
        assert max_len[self.lang_id] > 0
        init_size = len(self.pos)
        indices = np.arange(len(self.pos))
        indices = indices[self.lengths[indices] <= max_len[self.lang_id]]
        self.pos = self.pos[indices]
        if self.has_abstact:
            self.pos_abs = self.pos_abs[indices]
        self.lengths = self.pos[:, 1] - self.pos[:, 0]
    ...
    def select_data(self, a, b):
        ...
        if a < b:
            ...
            self.pos_abs = self.pos_abs[a:b]
            self.lengths = self.pos[:, 1] - self.pos[:, 0]
        else:
            self.pos = torch.LongTensor()
            self.pos_abs = torch.LongTensor()
    ...
    def get_batches_iterator(self, batches):
        ...
        def iterator():
            for sentence_ids in batches:
                ...
                yield self.batch_sentences(sent, self.lang_id), (None, None)
        def iterator_abs():
            for sentence_ids in batches:
                pos = self.pos[sentence_ids]
                pos_abs = self.pos_abs[sentence_ids]
                sent = [self.sent[a:b] for a, b in pos]
                sent_abs = [self.sent_abs[a:b] for a, b in pos_abs]
                yield self.batch_sentences(sent, self.lang_id), self.batch_sentences(sent_abs, self.lang_id)
        if self.has_abstact:
            return iterator_abs
        else:
            return iterator
...
class ParallelDataset(Dataset):
    ...
    def __init__(self, sent1, pos1, dico1, lang1_id, sent2, pos2, dico2, lang2_id, params, has_abs=False, sent_abs=None, pos_abs=None, abs_lang_id=0):
        ...
        if has_abs:
            ...
            self.sent_abs = torch.LongTensor([0, 0])
            self.has_abstact = False
            self.pos_abs = torch.LongTensor([[0, 1]]*self.pos1.shape[0])
        if not(abs_lang_id==lang1_id or abs_lang_id==lang2_id):
            logger.info('WARNING, abs_lang_id must be either lang1_id or lang2_id.')
        logger.info('abs_lang_id: %d' % abs_lang_id)
        logger.info('lang1_id: %d' % lang1_id)
        logger.info('lang2_id: %d' % lang2_id)
        self.abs_lang_id = abs_lang_id
        self.dico1 = dico1
        self.dico2 = dico2
    ...
    def remove_long_sentences(self, max_len):
        ...
        init_size = len(self.pos1)
        indices = np.arange(len(self.pos1))
        indices = indices[self.lengths1[indices] <= max_len[self.lang1_id]]
        indices = indices[self.lengths2[indices] <= max_len[self.lang2_id]]
        self.pos1 = self.pos1[indices]
        self.pos2 = self.pos2[indices]
        self.pos_abs = self.pos_abs[indices]
        self.lengths1 = self.pos1[:, 1] - self.pos1[:, 0]
    ...
    def get_batches_iterator(self, batches):
        ...
        def iterator():
            for sentence_ids in batches:
                ...
                sent_abs = [self.sent_abs[a:b] for a, b in pos_abs]
                yield self.batch_sentences(sent1, self.lang1_id), self.batch_sentences(sent_abs, self.abs_lang_id), self.batch_sentences(sent2, self.lang2_id)
        return iterator
