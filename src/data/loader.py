    ...
    if len(params.para_dataset) > 0:
        for (lang1, lang2), paths in params.para_dataset.items():
            ...
            for name, path in zip(['train', 'valid', 'test'], paths):
                ...
                assert (data1['positions'].shape[0] == data2['positions'].shape[0]) ,"data1['positions'].shape[0]:%d data2['positions'].shape[0]:%d"%(data1['positions'].shape[0], data2['positions'].shape[0])
                assert (data1['positions'].shape[1] == data2['positions'].shape[1]) ,"data1['positions'].shape[0]:%d data2['positions'].shape[0]:%d"%(data1['positions'].shape[1], data2['positions'].shape[1])
                ...
                if params.use_summary:
                    if lang1=='sw':
                        para_data = ParallelDataset(
                            ...
                            params,
                            has_abs=True,
                            sent_abs=data1['sentences_abs'], 
                            pos_abs=data1['positions_abs'],
                            abs_lang_id = params.lang2id[lang1]
                        )
                    elif lang2=='sw':
                        para_data = ParallelDataset(
                            data1['sentences'], data1['positions'], data['dico'][lang1], params.lang2id[lang1],
                            ...
                            params,
                            has_abs=True,
                            sent_abs=data2['sentences_abs'], 
                            pos_abs=data2['positions_abs'],
                            abs_lang_id = params.lang2id[lang2]
                        )
                else:
                    para_data = ParallelDataset(
                        data1['sentences'], data1['positions'], data['dico'][lang1], params.lang2id[lang1],
        ...
        logger.info('No para data found.')
...
def load_mono_data(params, data):
    ...
    for lang, paths in params.mono_dataset.items():
        ...
        for name, path in zip(['train', 'valid', 'test1', 'test2'], paths):
            if path == '':
                assert name != 'train'
                datasets.append((name, None))
                continue
...
def check_all_data_params(params):
    ...
    # max length
    max_lens = params.max_len.split(',')
    params.max_len = {}
    assert len(max_lens)==len(params.langs)
    for i in range(len(params.langs)):
        lang = params.langs[i]
        max_len = int(max_lens[i])
        assert max_len > 0
        params.max_len[params.lang2id[lang]]=max_len
...
def load_data(params, mono_only=False):
    ...
    for lang, v in data['mono'].items():
        for data_type in ['train', 'valid', 'test1', 'test2']:
