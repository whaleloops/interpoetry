...
def reload_pt_emb(path, dim):
    """
    home made: Reload pretrained embeddings from a PyTorch binary file.
    """
    from ..data import dictionary
    sys.modules['src.dictionary'] = dictionary
    logger.info("Reloading embeddings from %s ..." % path)
    ...
    logger.info("Reloaded %i embeddings." % len(vectors))
    word2id = {}
    with open('./data/vocab.txt') as f_vocab:
        con_vocab = f_vocab.readlines()
    for i in range(len(con_vocab)):
        word2id[con_vocab[i].strip()] = i
    # revision starts here
    return vectors.detach().numpy(), word2id
